...
LaserScanRange Particles::getLaserScan(maebot_pose_t poseA, maebot_laser_scan_t scanB, std::vector<maebot_pose_t>& poses, maebot_pose_t* mB)
{
    bool oneElt = findLeftRightPoses(scanB.utime, poses, mB);
    if(oneElt)
    {
...
bool Particles::findLeftRightPoses(int64_t time, std::vector<maebot_pose_t>& poses, maebot_pose_t* m)
{
    assert(!poses.empty());
    if(poses.size() == 1)
    {
